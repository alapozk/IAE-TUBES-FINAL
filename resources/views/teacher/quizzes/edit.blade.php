...
<div class="page">
  <div class="container">
    ...
    <div class="card">
      ...
      <form method="POST"
            action="{{ route('teacher.courses.quizzes.update', [$course->id, $quiz->id]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
          ...
          <select name="max_attempt" class="form-control">
            <option value="1" @selected($quiz->max_attempt == 1)>
              1x Attempt (Sekali saja)
            </option>
            <option value="2" @selected($quiz->max_attempt == 2)>
              2x Attempt
            </option>
          </select>
          <div class="hint">
        ...
        <div class="form-group">
          ...
          <input type="datetime-local"
                 ...
                 class="form-control"
                 value="{{ $quiz->deadline
                    ? \Carbon\Carbon::parse($quiz->deadline)->format('Y-m-d\TH:i')
                    : '' }}">
          <div class="hint">
        ...
        <div class="form-group">
          ...
          <select name="show_review" class="form-control">
            <option value="1" @selected($quiz->show_review)>
              Boleh lihat review
            </option>
            <option value="0" @selected(!$quiz->show_review)>
              Tidak boleh lihat review
            </option>
          </select>
          <div class="hint">
        ...
        <div class="form-group">
          ...
          <select name="is_published" class="form-control">
            <option value="1" @selected($quiz->is_published)>
              👁️ Dibuka untuk siswa
            </option>
            <option value="0" @selected(!$quiz->is_published)>
              🔒 Disembunyikan
            </option>
          </select>
        </div>
